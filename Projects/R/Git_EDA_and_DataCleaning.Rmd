...
```
To add some customization on axis labels, we'll plot again with ggplot2
```{r}
library(ggplot2)
...
```
There is a clear outlier with the max income being 7,000,000 despite the max loan amount being set to $3500. Let's check the distribution of the annual income feature

```{r}
boxplot(BNPL_clients$annual_inc)

```
The distribution is so skewed, we hardly see the box plot. 
Let's move to ggplot2 for more control.
...
ggplot(BNPL_clients, aes(y=annual_inc)) +
...
```
As we can clearly see, the outliers for a reasonable annual income are off the charts. 
As a test, let's calculate the quartiles and limit to the 3rd quartile to inspect the distribution
```{r}
quantile(BNPL_clients$annual_inc)
```
...
BNPL_anninc_3rdQ <- BNPL_clients[(BNPL_clients$annual_inc <= 67000),]

ggplot(BNPL_anninc_3rdQ, aes(y=annual_inc)) +
  geom_boxplot(outlier.colour = "blue", outlier.shape = 16, 
               outlier.size = 3, notch = FALSE) +
  scale_y_continuous(
    limits = c(0, 100000),
...
```
```{r}
nrow(BNPL_anninc_3rdQ)
```
Reducing the annual income to the 3rd quartile has a better distribution than previously seen, but reduces the number of observations to under 45,000. 
So balance out annual income, we'll set the max of annual income to $100,000. 
This level of income is likely to still engage in BNPL services.
